  ...
  <div class="results">
    <div class="filter">
      <div class="filter-item">
        ...
        <options :items="['Any', '1', '2', '3', '4', '5+']" param="bedroom" id="filter-lease-bedrooms" class="custom-dropdown" v-on:option-select="update"></options>
      </div>
      <div class="filter-item">
        <div class="pre-text"><i class="icon-bath-outline"></i></div>
        <options :items="['Any', '1', '2', '3', '4', '5+']" param="bathroom" id="filter-lease-bathrooms" class="custom-dropdown" v-on:option-select="update"></options>
      </div>
      <div class="filter-item">
        <div class="pre-text"><i class="icon-bath-outline"></i></div>
        <options :items="['Any', '1', '2', '3', '4', '5+']" param="parking" id="filter-lease-parking" class="custom-dropdown" v-on:option-select="update"></options>
      </div>
      <div class="filter-item">
        <div class="pre-text"><i class="icon-bath-outline"></i></div>
        <options :items="['Any', '100', '200', '300', '400', '500', '600', '700', '800', '900', '1000+']" param="price" id="filter-lease-price" class="custom-dropdown" v-on:option-select="update"></options>
      </div>
    </div>
    <spinner :active="network" spinner="spinner" color="rgba(239,98,108,1)"/>
    <div v-if="re.apartments" class="results">
      <div class="row">
        <div class="null" v-if="re.apartments.content.length <= 0">Oops... I can't see any apartments for lease.</div>
...
export default {
  ...
  methods: {
    initialise: function () {
      var vm = this
      this.$http('building/id/' + vm.id + '/lease?page=' + vm.re.filter.page).then(r => {
    ...
    update: function (item, param) {
      var vm = this
      vm.network = true
      var base = 'building/id/' + vm.id + '/lease'
      var additions = '?'
